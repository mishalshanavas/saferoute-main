...
public class GetNearbyContributionsHandler implements RequestHandler<APIGatewayProxyRequestEvent, APIGatewayProxyResponseEvent> {
    ...
    public APIGatewayProxyResponseEvent handleRequest(APIGatewayProxyRequestEvent request, Context context) {
        try {
            ...
            if (qs == null) qs = new HashMap<>();
            double lat = Double.parseDouble(qs.getOrDefault("latitude", "0"));
            double lon = Double.parseDouble(qs.getOrDefault("longitude", "0"));
            double radius = Double.parseDouble(qs.getOrDefault("radius", "2"));
            List<Contribution> list = repo.findNearby(lat, lon, radius);
            ...
            return new APIGatewayProxyResponseEvent()
                    .withStatusCode(400)
                    .withHeaders(cors())
                    .withBody("{\"message\":\"Invalid query params\"}");
        }
    }
